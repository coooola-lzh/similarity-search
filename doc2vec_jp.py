...
print("Load dictionary for url to indices...", flust=True)
t1 = time.time()
...
print("Start loading the corpus...", flust=True)
t1 = time.time()
train_corpus = list(read_corpus(text_path))
t2 = time.time()
print("Finished loading the corpus, took {:.3f} secs".format(t2 - t1), flust=True)
total_word_cnt = sum([len(doc.words) for doc in train_corpus])
print("The corpus has {} instances in total, {} words\n".format(len(train_corpus), total_word_cnt), flust=True)
...
print("Start to train the model...", flust=True)
...
t1 = time.time()
print("Start to pickle the training corpus.", flust=True)
